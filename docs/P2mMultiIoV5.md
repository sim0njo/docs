...
## Intro
The Em16Am4Jrm device is the combination 3 indiviual PHC bus modules, 1 input module with 16 channels (940/24 EM) and 2 output modules with can be 
configured as either 8 outputs (940/4 AM) or 4 shutters (940 JRM).
...
## Hardware
The hardware comprises 2 PCB's, the bottom PCB contains the input/output hardware, while the upper PCB contains the ESP32-C3 controller that interacts with the 
RS485 module bus and the Wifi network. The complete device comes in a 9 mod dinrail housing. 
...
- Now P2M is connected to your wireless network, you need to find out it's IP address.

- Press the <b>WIFI</b> button, now the last number of the IP address (192.168.0.x) will be
...
Look for an entry showing 'openHC', enter the shown IP address in a web browser, you should see the [Main Menu](#main-menu) webpage.
...
- [Configure Left Output Module](#configure-phc-interfaces) and put PHC Mgmt Interface Admin State to 'Enabled', then press Save  
...
- **MQTT Telemetry**: When enabled, P2M will periodically send MQTT telemetry messages with system vital information.  

- **Save**: Press this button to save settings, then [reboot](#reboot) P2M to let changes take effect.  


###Configure Network

<img style="float:right;width:352px;height:304px" src="../img/p2m-config-netw-v5.jpg"></img>
...
Ethernet mode: P2M will utilise a wired connection if the hw version provides this. Otherwise it will fallback to STA mode.  

- **Remote AP SSID/Remote AP Password**: P2M will use these credentials to connect in STA-mode to the Wifi network.
When you leave Remote AP SSID empty, P2M will revert to AP-mode.  

- **Save**: Press this button to save settings, then [reboot](#reboot) P2M to let changes take effect.  

<b style="color:red">Lockout:</b>
If you entered a wrong Remote AP SSID/Password or the remote AP fails, connecting to the Wifi network will fail. 
P2M will continiously blink the blue LED and retry connecting after 30 seconds until connection succeeds. 
This can cause a lockout where you cannot access P2M.
...
<img style="display:block;float:right;width:352px;height:668px" src="../img/p2m-config-mqtc-v5.jpg"></img>  

- **Broker IP Address/Broker IP Port**: Mandatory. Enter the MQTT Broker IP address and port (default 1883).  

- **ClientId**: Required, max 24 characters. A unique name across all clients that connect to the same broker.
...
- **Start Now/Stop Now**: Using this button you can instantly change the Oper State of SRSD, this does not change the Admin State.  
...
This interface is used to send commands to a remote STM module (v1/v2/v3) when P2M is running in Proxy mode.

- **Remote STM Mode**: Select the appropriate remote STM in your PHC system.  

- **Remote STM IP Address/Remote STM IP Port**: IP address and port on which the remote STM in your PHC system is reachable.
...
- **Remote STM Mgmt Address**: This is the remote STM management address as defined in the Systemsoftware for the remote STM, normally this is 0.  
...
The MQTT client will prepend '&lt;tx-topic-prefix>/' to the topic explained below.  
...
- **Download File**: Select the desired file in the dropdown list, then click <b>Download File</b> to start download to your PC.  

- **Clear File**: Select the desired file in the dropdown list, then click <b>Clear File</b>. You will be prompted to confirm the clear action.
&nbsp;  
&nbsp;  
&nbsp;  
&nbsp;  
...
You can also enter part of the command followed by '?' to show relevant commands
...
admin save          save settings to NV ram of the ESP32
...
save                save settings to NV ram of the ESP32
