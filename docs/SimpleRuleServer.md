...
### Event Sources
Events are generated by a source, these can be internal or external to the SRS as explained here.
...
matches any of the configured SRS RX-Topic-Prefixes.
...
### Event Filtering
For every incoming event the SRS will iterate over the list of state/action-rules in the order they were defined.
...
### Init Rule
When the SRS starts it will execute all init rules without the need for an event to trigger the rule.
...
### Exit Rule
When the SRS stops it will execute all exit rules without the need for an event to trigger the rule.

An exit rule executes any action but note that some actions make no sense (state/timer/clock) as the SRS will stop any activity.
```
exit:{
...
the SRS will parse them as follows.
